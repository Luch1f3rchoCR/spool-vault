...
**Naming vigente · actualización del fundador (2026-09-25/26):** **Proper V1** es la marca de trabajo autorizada para la app. Esta instrucción reemplaza la prohibición anterior de implementarla, pero el clearance comercial/jurídico sigue pendiente. Spool Vault queda como referencia técnica/histórica; Meld/Strata continúan descartados. Promesa: **Simple para empezar. Potente para crecer.** Tagline: **Everything behind your prints.** Stone Collective conserva el rol secundario; falta su SVG oficial, por lo que no se recrea su logo. No renombrar infraestructura. [docs/BRAND_STATUS.md](./docs/BRAND_STATUS.md) prevalece sobre antecedentes incompatibles.
...
Publicado: Nueva orden funciona sin compras previas y admite filamentos nuevos junto con compras existentes compatibles. Se conserva presentación con/sin spool y precio del producto; el spool físico se asigna por el flujo existente. `create_purchase_order_v3` compone rollos, proveedor, historial, partidas y pago en una transacción, con registro persistente de reintentos y rechazo de payload cambiado. La migración local `20260910135214_purchase_order_new_filaments.sql` se aplicó en Supabase como `20260911173048`, antes de publicar la interfaz. No reaplicarla por la diferencia de timestamps.

Pruebas aprobadas: esquema completo desde cero en PostgreSQL 17 aislado, reversión, mezcla de compras antiguas/nuevas, aislamiento por usuario, costos de spool y reintentos; navegador simulado en 320/390/1280 px, primera orden, pérdida de respuesta, intento de modificar una operación ya guardada, detalle, inventario y recarga. Regresión de navegación/spools/compras también aprobada. Publicación autorizada y verificada: RLS activo, función invoker sin acceso anónimo y registro de solicitudes sin permisos de actualización/eliminación para usuarios. Humo de producción sin sesión: Compras → Nueva orden → Filamento nuevo y consola sin errores; sin guardar. No se enviaron correos ni se modificó inventario real. Pendiente prioritario: confirmar el recorrido real de activación del primer invitado antes de ampliar el grupo.

## Bloque publicado · 9 de septiembre

Cambio funcional de ese bloque: PR #23, merge `a431c42449ddd24edbef594c2236655b43f12223`, publicado en Vercel Production. Incluye impresoras en Perfil, cuestionario único de primer ingreso y correcciones de costos. Supabase confirmó `printer_cost_safety` (20260909203554) y `tester_first_visit` (20260909203604). Compilación, navegador simulado y pruebas SQL locales aprobadas. Humo de producción sin sesión: proyectos e impresoras disponibles, sin errores de consola; esto no equivale a una activación real.

Prioridad: comprobar correo, enlace, cuestionario y activación de la primera invitación ya enviada. Todavía no se confirmó ese recorrido real; esperar su resultado antes de invitar al resto del grupo de 8–10. No se enviaron correos durante la publicación. El bloque posterior de filamentos desde órdenes ya está publicado; ver continuidad actual arriba.
